...
</code></pre><h1 id="rendering-with-multiple-contexts" tabindex="-1">RENDERING WITH MULTIPLE CONTEXTS <a class="header-anchor" href="#rendering-with-multiple-contexts" aria-hidden="true">#</a></h1><p>Use multiple text contexts if you need to render debug text in different sokol-gfx render passes, or want to render text to different layers in the same render pass, each with its own set of parameters.</p><p>To create a new text context call:</p><pre><code>sdtx_context ctx = sdtx_make_context(&amp;(sdtx_context_desc_t){ ... });
...
</code></pre><h1 id="using-your-own-font-data" tabindex="-1">USING YOUR OWN FONT DATA <a class="header-anchor" href="#using-your-own-font-data" aria-hidden="true">#</a></h1><p>Instead of the built-in fonts you can also plug your own font data into sokol-debugtext by providing one or several sdtx_font_desc_t structures in the sdtx_setup call.</p><p>For instance to use a built-in font at slot 0, and a user-font at font slot 1, the sdtx_setup() call might look like this:</p><pre><code>sdtx_setup(&amp;sdtx_desc_t){
    .fonts = {
        [0] = sdtx_font_kc853(),
        [1] = {
            .data = {
...
...
</code></pre><p>If no overrides are provided, puts will be used on most platforms. On Android, __android_log_write will be used instead.</p><h1 id="license" tabindex="-1">LICENSE <a class="header-anchor" href="#license" aria-hidden="true">#</a></h1><p>zlib/libpng license</p><p>Copyright (c) 2020 Andre Weissflog</p><p>This software is provided &#39;as-is&#39;, without any express or implied warranty. In no event will the authors be held liable for any damages arising from the use of this software.</p><p>Permission is granted to anyone to use this software for any purpose, including commercial applications, and to alter it and redistribute it freely, subject to the following restrictions:</p><pre><code>1. The origin of this software must not be misrepresented; you must not
