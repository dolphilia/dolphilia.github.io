import{_ as e,c as o,o as i,a as n}from"./app.1650bd79.js";const f=JSON.parse('{"title":"FEATURE OVERVIEW:","description":"","frontmatter":{},"headers":[],"relativePath":"translation/sokol/imgui.md","lastUpdated":1676126774000}'),t={name:"translation/sokol/imgui.md"},a=n(`<p>sokol_imgui.h -- drop-in Dear ImGui renderer/event-handler for sokol_gfx.h</p><p>Project URL: <a href="https://github.com/floooh/sokol" target="_blank" rel="noreferrer">https://github.com/floooh/sokol</a></p><p>Do this: #define SOKOL_IMPL or #define SOKOL_IMGUI_IMPL</p><p>before you include this file in <em>one</em> C or C++ file to create the implementation.</p><p>NOTE that the implementation can be compiled either as C++ or as C. When compiled as C++, sokol_imgui.h will directly call into the Dear ImGui C++ API. When compiled as C, sokol_imgui.h will call cimgui.h functions instead.</p><p>NOTE that the formerly separate header sokol_cimgui.h has been merged into sokol_imgui.h</p><p>The following defines are used by the implementation to select the platform-specific embedded shader code (these are the same defines as used by sokol_gfx.h and sokol_app.h):</p><p>SOKOL_GLCORE33 SOKOL_GLES2 SOKOL_GLES3 SOKOL_D3D11 SOKOL_METAL SOKOL_WGPU</p><p>Optionally provide the following configuration defines before including the implementation:</p><p>SOKOL_IMGUI_NO_SOKOL_APP - don&#39;t depend on sokol_app.h (see below for details)</p><p>Optionally provide the following macros before including the implementation to override defaults:</p><p>SOKOL_ASSERT(c) - your own assert macro (default: assert(c)) SOKOL_IMGUI_API_DECL- public function declaration prefix (default: extern) SOKOL_API_DECL - same as SOKOL_IMGUI_API_DECL SOKOL_API_IMPL - public function implementation prefix (default: -)</p><p>If sokol_imgui.h is compiled as a DLL, define the following before including the declaration or implementation:</p><p>SOKOL_DLL</p><p>On Windows, SOKOL_DLL will define SOKOL_IMGUI_API_DECL as __declspec(dllexport) or __declspec(dllimport) as needed.</p><p>Include the following headers before sokol_imgui.h (both before including the declaration and implementation):</p><pre><code>sokol_gfx.h
sokol_app.h     (except SOKOL_IMGUI_NO_SOKOL_APP)
</code></pre><p>Additionally, include the following headers before including the implementation:</p><p>If the implementation is compiled as C++: imgui.h</p><p>If the implementation is compiled as C: cimgui.h</p><h1 id="feature-overview" tabindex="-1">FEATURE OVERVIEW: <a class="header-anchor" href="#feature-overview" aria-hidden="true">#</a></h1><p>sokol_imgui.h implements the initialization, rendering and event-handling code for Dear ImGui (<a href="https://github.com/ocornut/imgui" target="_blank" rel="noreferrer">https://github.com/ocornut/imgui</a>) on top of sokol_gfx.h and (optionally) sokol_app.h.</p><p>The sokol_app.h dependency is optional and used for input event handling. If you only use sokol_gfx.h but not sokol_app.h in your application, define SOKOL_IMGUI_NO_SOKOL_APP before including the implementation of sokol_imgui.h, this will remove any dependency to sokol_app.h, but you must feed input events into Dear ImGui yourself.</p><p>sokol_imgui.h is not thread-safe, all calls must be made from the same thread where sokol_gfx.h is running.</p><h1 id="howto" tabindex="-1">HOWTO: <a class="header-anchor" href="#howto" aria-hidden="true">#</a></h1><p>--- To initialize sokol-imgui, call:</p><pre><code>simgui_setup(const simgui_desc_t* desc)
...
</code></pre><h1 id="memory-allocation-override" tabindex="-1">MEMORY ALLOCATION OVERRIDE <a class="header-anchor" href="#memory-allocation-override" aria-hidden="true">#</a></h1><p>You can override the memory allocation functions at initialization time like this:</p><pre><code>void* my_alloc(size_t size, void* user_data) {
    return malloc(size);
}
...
...
</code></pre><p>If no overrides are provided, malloc and free will be used.</p><p>This only affects memory allocation calls done by sokol_imgui.h itself though, not any allocations in Dear ImGui.</p><h1 id="license" tabindex="-1">LICENSE <a class="header-anchor" href="#license" aria-hidden="true">#</a></h1><p>zlib/libpng license</p><p>Copyright (c) 2018 Andre Weissflog</p><p>This software is provided &#39;as-is&#39;, without any express or implied warranty. In no event will the authors be held liable for any damages arising from the use of this software.</p><p>Permission is granted to anyone to use this software for any purpose, including commercial applications, and to alter it and redistribute it freely, subject to the following restrictions:</p><pre><code>1. The origin of this software must not be misrepresented; you must not
...
distribution.
</code></pre>`,46),s=[a];function r(l,d,p,h,u,m){return i(),o("div",null,s)}const _=e(t,[["render",r]]);export{f as __pageData,_ as default};
